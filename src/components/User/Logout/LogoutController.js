...
class Logout {
  async handler (request, response) {
    try {
      logger.info(`Usuário:${request.user.id} está tentando realizar logout na aplicação.`)
      const logoutRequestDTO = new LogoutRequestDTO(request.token)
      const logoutService = new LogoutService()
      await logoutService.execute(logoutRequestDTO)
      logger.info(`Usuário:${request.user.id} conseguiu realizar logout na aplicação.`)
