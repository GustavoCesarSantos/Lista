...
describe('LIST ANNOTATIONS SERVICE UNIT TEST', () => {
  test('Should return a valid annotations with out throw an erro', async () => {
    ...
    const listAnnotationsService = new ListAnnotationsService(annotationRepositoryFake)
    await expect(listAnnotationsService.execute(annotation)).resolves.not.toThrow()
  })
})
