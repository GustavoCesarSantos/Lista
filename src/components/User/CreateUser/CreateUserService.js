...
class CreateUserService {
  ...
  async execute (createUserRequestDTO) {
    ...
    await user.isValid()
    const userExists = await this.userRepository.findMany({ email: user.email })
    if (userExists.length > 0) throw new ErrorHandler('Usuário já cadastrado.', 400)
    const hashedPassword = await bcryptHelper.encryptPassword(user.password)
    user.password = hashedPassword
    await this.userRepository.create(user)

    const [{ id }] = await this.userRepository.findMany({ email: user.email })
    const idToken = tokenHelper.createToken({ id })
    user.id = idToken
    const verificationEmail = new VerificationEmail(user)
    await verificationEmail.sendEmail().catch(error => { throw new ErrorHandler(error, 500) })
  }
}
